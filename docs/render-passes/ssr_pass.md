...
Source: /engine/src/render/passes/ssr_pass.zig

## How It Should Work (Design Contract)
- Trace approximate reflections in screen/depth space from reflective pixels.
- Reject invalid hits robustly to avoid temporal sparkle and edge leaks.
- Constrain sample budget per pixel for frame pacing predictability.
- Blend reflection contribution with scene color in a stable way.

## How It Does Work Today (Code Walkthrough)
- runRows() calls SSR kernel with current projection/camera inputs.
- runPipeline() row-dispatches reflection work over scene height.
- Uses pass config and kernel rules to determine reflection ray steps/hits.
- Writes reflection-composited result into scratch/destination targets.
...
- ../kernels/ssr_kernel.zig

## Key Entry Points in This File
- L11: pub fn runRows(
- L46: pub fn runPipeline(self: anytype, projection: anytype, scene_height: usize, comptime noop_job_fn: fn (*anyopaque) void) void {
