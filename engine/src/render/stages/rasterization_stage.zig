...
pub const TileRasterState = struct {
    ...
    cached_prepared_vertex_depths: ?*const std.ArrayListUnmanaged(?[3]f32) = null,
    ...
    if (shouldUseWorkerTiles(raster_mode, job_sys, active_tiles.len)) {
        ...
        while (chunk_index < chunk_count and start < active_tiles.len) : (chunk_index += 1) {
            ...
            state.tile_chunk_job_contexts.items[chunk_index] = .{
                ...
                .cached_prepared_vertex_depths = if (state.cached_prepared_vertex_depths) |items| items.items else null,
...
pub const RasterTileJobContext = struct {
    ...
    cached_prepared_vertex_depths: ?[]const ?[3]f32 = null,
...
const TilePreparedGouraudBatch = struct {
    ...
    vertex_depths: [max_tile_gouraud_batch]?[3]f32 = undefined,
    len: usize = 0,
    ...
    inline fn vertexDepthSlice(self: *const TilePreparedGouraudBatch) []const ?[3]f32 {
        return self.vertex_depths[0..self.len];
    }
};
...
fn rasterTileChunk(ctx: *const RasterTileChunkJobContext) void {
    ...
    while (index < ctx.end) : (index += 1) {
        var tile_ctx = RasterTileJobContext{
            ...
            .cached_prepared_vertex_depths = ctx.cached_prepared_vertex_depths,
    ...
    if (ctx.cached_prepared_tile_ranges) |prepared_tile_ranges| {
        ...
        if (range.len > 0) {
            direct_primitives.drawPreparedGouraudTriangleBlock(
                ...
                ctx.cached_prepared_vertex_depths.?[range.start .. range.start + range.len],
            );
        }
...
inline fn tryAppendPreparedGouraud(batch: *TilePreparedGouraudBatch, prepared_entry: ?direct_draw_list.DrawList.PreparedGouraudEntry) bool {
    ...
    batch.vertex_depths[batch.len] = entry.vertex_depths;
    batch.len += 1;
    return true;
}
...
inline fn flushPreparedGouraudBatch(
    ...
    direct_primitives.drawPreparedGouraudTriangleBlock(target, batch.triangleSlice(), batch.preparedSlice(), batch.depthSlice(), batch.vertexDepthSlice());
    batch.clear();
}
