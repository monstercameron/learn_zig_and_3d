...
pub fn load(allocator: std.mem.Allocator, path: []const u8) !Gltf {
    ...
    var result = if (std.mem.eql(u8, ext, ".glb")) blk: {
        break :blk try parseGlb(allocator, bytes);
    } else if (std.mem.eql(u8, ext, ".gltf")) blk: {
        const base_dir = try std.fs.path.dirnameAlloc(allocator, path);
        defer allocator.free(base_dir);
        break :blk try parseGltfJson(allocator, bytes, base_dir);
...
fn accessorAsVec4Slice(gltf: *const Gltf, accessor_index: usize) ![]Quat {
    const view = try getAccessorView(gltf, accessor_index);
